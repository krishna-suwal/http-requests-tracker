...
namespace HRT\Database\QueryBuilder;
...
class QueryBuilder implements BuilderContract {
	 ...
	 * @var array
	 */
	public $beforeQueryCallbacks = array();
	...
	public function selectSub( $query, $as ) {
		array( $query, $bindings ) = $this->createSub( $query );
	...
	public function where( $column, $operator = null, $value = null, $boolean = 'and' ) {
		...
		array( $value, $operator ) = $this->prepareValueAndOperator(
			$value,
			$operator,
			func_num_args() === 2
		);
		...
		if ( $this->isQueryable( $column ) && ! is_null( $operator ) ) {
			array( $sub, $bindings ) = $this->createSub( $column );
		...
		if ( $this->invalidOperator( $operator ) ) {
			array( $value, $operator ) = array( $operator, '=' );
		}
	...
	public function whereColumn( $first, $operator = null, $second = null, $boolean = 'and' ) {
		...
		// we will set the operators to '=' and set the values appropriately.
		if ( $this->invalidOperator( $operator ) ) {
			array( $second, $operator ) = array( $operator, '=' );
		}
	...
	public function whereIn( $column, $values, $boolean = 'and', $not = false ) {
		...
		if ( $this->isQueryable( $values ) ) {
			array( $query, $bindings ) = $this->createSub( $values );

			$values = array( new Expression( $query ) );
	...
	public function orderBy( $column, $direction = 'asc' ) {
		if ( $this->isQueryable( $column ) ) {
			array( $query, $bindings ) = $this->createSub( $column );
		...
		$this->{$this->unions ? 'unionOrders' : 'orders'}[] = array(
			'column'    => $column,
			'direction' => $direction,
		);
	...
	public function reorder( $column = null, $direction = 'asc' ) {
		...
		$this->bindings['unionOrder'] = array();
	...
	public function applyBeforeQueryCallbacks() {
		...
		$this->beforeQueryCallbacks = array();
	}
	 ...
	 */
	public function cursorPaginate( $perPage = 15, $columns = array( '*' ), $cursorName = 'cursor', $cursor = null ) {
	...
	protected function ensureOrderForCursorPagination( $shouldReverse = false ) {
		...
		if ( $shouldReverse ) {
			$this->orders = collect( $this->orders )->map(
				function ( $order ) {
	...
	protected function runPaginationCountQuery( $columns = array( '*' ) ) {
		...
		$without = $this->unions ? array( 'orders', 'limit', 'offset' ) : array( 'columns', 'orders', 'limit', 'offset' );

		return $this->cloneWithout( $without )
					->cloneWithoutBindings( $this->unions ? array( 'order' ) : array( 'select', 'order' ) )
					->setAggregate( 'count', $this->withoutSelectAliases( $columns ) )
					->get()->all();
	}
	...
	protected function cloneForPaginationCount() {
		return $this->cloneWithout( array( 'orders', 'limit', 'offset' ) )
					->cloneWithoutBindings( array( 'order' ) );
	}
	...
	public function pluck( $column, $key = null ) {
		...
		$queryResult = $this->onceWithColumns(
			is_null( $key ) ? array( $column ) : array( $column, $key ),
			function () {
	...
	public function aggregate( $function, $columns = array( '*' ) ) {
		$results = $this->cloneWithout( $this->unions || $this->havings ? array() : array( 'columns' ) )
						->cloneWithoutBindings( $this->unions || $this->havings ? array() : array( 'select' ) )
	...
	public function insertUsing( array $columns, $query ) {
		...
		array( $sql, $bindings ) = $this->createSub( $query );
