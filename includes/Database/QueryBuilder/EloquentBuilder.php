...
// use HRT\Contracts\Query\Builder as BuilderContract;
...
class EloquentBuilder {
	...
	public function orWhere( $column, $operator = null, $value = null ) {
		array( $value, $operator ) = $this->query->prepareValueAndOperator(
			$value,
			$operator,
			func_num_args() === 2
		);
	 ...
	 * @return \Illuminate\Database\Eloquent\Model|static
	 */
	public function firstOrNew( array $attributes = array(), array $values = array() ) {
	 ...
	 * @return \Illuminate\Database\Eloquent\Model|static
	 */
	public function firstOrCreate( array $attributes = array(), array $values = array() ) {
		if ( ! is_null( $instance = $this->where( $attributes )->first() ) ) {
			return $instance;
		}

		return tap(
			$this->newModelInstance( array_merge( $attributes, $values ) ),
			function ( $instance ) {
				$instance->save();
			}
		);
	...
	public function valueOrFail( $column ) {
		return $this->firstOrFail( array( $column ) )->{Str::afterLast( $column, '.' )};
	}
	 ...
	 */
	public function cursorPaginate( $perPage = null, $columns = array( '*' ), $cursorName = 'cursor', $cursor = null ) {
		$perPage = $perPage ?: $this->model->getPerPage();
	...
	protected function addUpdatedAtColumn( array $values ) {
		...
		$values = array_merge(
			array( $column => $this->model->freshTimestampString() ),
			$values
		);
	...
	protected function addTimestampsToUpsertValues( array $values ) {
		...
		foreach ( $columns as $column ) {
			foreach ( $values as &$row ) {
				$row = array_merge( array( $column => $timestamp ), $row );
			}
		}
	...
	public function scopes( $scopes ) {
		...
		foreach ( Arr::wrap( $scopes ) as $scope => $parameters ) {
			...
			if ( is_int( $scope ) ) {
				array( $scope, $parameters ) = array( $parameters, array() );
			}
	 ...
	 * @return mixed
	 */
	protected function callScope( callable $scope, array $parameters = array() ) {
	 ...
	 * @return mixed
	 */
	protected function callNamedScope( $scope, array $parameters = array() ) {
	...
	public function with( $relations, $callback = null ) {
		if ( $callback instanceof Closure ) {
			$eagerLoad = $this->parseWithRelations( array( $relations => $callback ) );
		} else {
	...
	protected function parseWithRelations( array $relations ) {
		...
		foreach ( $relations as $name => $constraints ) {
			...
			if ( is_numeric( $name ) ) {
				...
				array( $name, $constraints ) = Str::contains( $name, ':' )
